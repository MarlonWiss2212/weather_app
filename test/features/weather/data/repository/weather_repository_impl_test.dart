...
@GenerateMocks([WeatherServiceImpl])
void main() {
  final MockWeatherServiceImpl mockService = MockWeatherServiceImpl();
  ...
  group("getWeatherData function tests", () {
    test(
        "should return WeatherForecastModel if service returns WeatherForecastModel",
        () async {
      ...
      when(
        mockService.getWeatherData(params: params),
      ).thenAnswer((_) async => model);

      // call function to test
      final response = await repository.getWeatherData(params: params);

      expect(response, DataState<WeatherForecastModel>.success(model));
    });

    test(
        "should return NoAPIResponseFailure if service throws NoAPIResponseException",
        () async {
      const params = GetWeatherWithLocationParams(
        lat: 33.44,
        lon: 33.44,
      ...
      expect(
        response,
        DataState<WeatherForecastModel>.failure(const NoAPIResponseFailure()),
      );
    });

    test(
        "should return ConvertingFailure if service throws ConvertingException",
        () async {
      const params = GetWeatherWithLocationParams(
        lat: 33.44,
        lon: 33.44,
      ...
      expect(
        response,
        DataState<WeatherForecastModel>.failure(const ConvertingFailure()),
      );
    });

    test(
        "should return ServerFailure if service throws DioException exceptions",
        () async {
      ...
      expect(
        response,
        DataState<WeatherForecastModel>.failure(
    ...
    test("should catch all exceptions", () async {
      ...
      expect(
        response,
        DataState<WeatherForecastModel>.failure(const UnkownFailure()),
      );
    });
  });
}
